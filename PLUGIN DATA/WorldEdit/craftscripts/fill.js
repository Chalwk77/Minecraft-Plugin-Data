/*
 * Fill Brush CraftScript for WorldEdit
...
var bSize = argv.length > 1 ? (argv[1]) : 4;
...
if (argv.length < 2)  {
	...
	tool.setBrush(brush,"worldedit.brush.fill");
	player.printError("Fill brush bound to " + ItemType.toHeldName(player.getItemInHand()) + "." );
...
function CommandText()  {
	...
	player.print(" ");
	player.print("Fill Brush Script by inHaze");
	player.printError("Usage: /cs fill <size><faces><strength>");
	player.print(" ");
	player.print("size - Brush radius size.");
	player.print("faces - Min # of exposed faces needed to trigger fill. (1-6)");
	player.print("strength - Brush strength multiplier.");
	player.print(" ");
	player.print("Use '/cs fill 0' to create with default settings.");
	return true;
}
